       ...
       $get_user_report = "SELECT RS.assigned_user_id AS assigned_user_id,U.user_name ,U.is_admin,
        ...
        while ($rec_user_report = $db->fetchByAssoc($row_user_report)) {
            ...
            if ($is_admin != '1') {
                ...
                $user_team_list = " AND O.assigned_user_id='$user_report_id' ";
                ...
                if (!empty($user_roles[0])) {
                    foreach ($user_roles[0] as $k => $v) {
                        if ($v == 'Regional Manager') {
                            ...
                            $user_team_list = " AND O.assigned_user_id IN ('" . implode("','", $active_users) . "') ";
...
EOD;
           ...
           // $note_msg->AddCC('mangesh@simplecrm.com.sg');
            ...
            if ($total_rows > 0) {
                ...
                $note_msg->AddAddress($user_email);
				//$note_msg->AddAddress('pratik@simplecrm.com.sg');
				//$note_msg->AddCC('malathir@squarefoot.co.in');
				//$note_msg->AddBCC('pratik@simplecrm.com.sg');
				//print_r($user_email);
				//exit;
            ...
            if ($frequency == 'Daily') {
                ...
                $end_date = date("Y-m-d H:i:s", strtotime($actual_date . ' 23:56:00'));
