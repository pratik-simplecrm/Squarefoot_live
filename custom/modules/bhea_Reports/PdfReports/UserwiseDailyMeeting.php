    ...
    function callme($assigned_user_id) {
        ...
        while ($rec_user_report = $db->fetchByAssoc($row_user_report)) {
            ...
            if ($is_admin != '1') {
                ...
                $user_team_list = " AND M.assigned_user_id='$user_report_id' ";
                ...
                if (!empty($user_roles[0])) {
                    foreach ($user_roles[0] as $k => $v) {
                        if ($v == 'Regional Manager') {
                            ...
                            $user_team_list = " AND M.assigned_user_id IN ('" . implode("','", $active_users) . "') ";
...
//				LEFT JOIN securitygroups_records SG ON M.id = SG.record_id and SG.deleted=0
            $GLOBALS['log']->fatal($query, "Meeting query");
...
EOD;
            ...
            //Main Query
            $query = "SELECT DISTINCT M.id , IFNULL(M.id,'') id,
...
EOD;
          ...
          //  $note_msg->AddCC('vivek@simplecrm.com.sg');
