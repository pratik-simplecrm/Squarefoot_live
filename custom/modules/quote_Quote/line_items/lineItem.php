...
<script type="text/javascript">
	...
	// added one extra parameter Sqm_Box (written by pratik and anjali on 15072019
    function addRow(count, quantity, prodName, productId, code, uom, amount, tax, discount, type, recordId, groupType, serviceTax, serviceTaxVal, Prod_Spec, pliID, Sqm_Box) {
		...
		//written by pratik and anjalion 15072019 end
...
//alert(uom);
        ...
        /***
            Added by Saurabh 8th Jan 2019
            Reflect Amount with new item added
        ****/
        setTimeout(function(){ calculate(row, count);  }, 1500);
    }
                 ...
                 $.ajax({
                    ...
                    success: function (result) {
                        if(result =='yes')
                        {
                            ...
                            alert("Ammended Unit Price should be greater than actual unit price of the selected product");
                             //$('#price_'+table_name).val('');
                             // $('#SAVE_HEADER').attr('disabled',true);
                             // $('#SAVE_HEADER').attr('disabled',true);
                             $('[id=SAVE_HEADER]').prop("disabled", true);
                             calculate(table_name,count);
                            return false;
                        }
                        else
                        {
                            $('[id=SAVE_HEADER]').prop("disabled", false);
                            return true;
                        }
...
//         }
    ...
    function calculate(table_name, count) {
		...
		if(structure_details)
		{
                    ...
                    /***
                        Added by Saurabh 9th Jan 2019
                        Reflect  Grand Total Amount with new item removed
                    ****/
                    if (isNaN(Total)) {
                        Total = 0;
                    }
