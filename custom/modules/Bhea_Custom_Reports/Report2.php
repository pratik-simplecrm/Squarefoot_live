...
$url = $sugar_config['site_url'];{echo $url;}?>"/>
...
foreach ($thing as $key => $value) {
	...
	<table border ='2' width="100%" cellpadding="2" cellspacing="2" class="imagetable">
		...
		<td style="background:#C0C0C0;">Exp. Opp. Closed to date&nbsp;&nbsp;</td>
		...
		<td style="background:#C0C0C0;">Active/ Inactive</td>
...
for ($i = 0; $i < ($count - 1); $i++) {
	...
	if ($content[20][$i] == 'Inactive' || $content[20][$i] == '') {
		if (($content[10][$i] != '0') && ($content[4][$i] != '0') && ($content[6][$i] != '0') || true) {?>
			...
			<?php }} else if ($content[20][$i] == 'Active') {?>
