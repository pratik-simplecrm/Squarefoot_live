...
{{if $fields[$colData.field.name].name!="email1"}}col-sm-2{{/if}}" scope="col" style="text-align:left" >
...
{{if $fields[$colData.field.name].name=="account_name" || $fields[$colData.field.name].name=="sales_stage" || $fields[$colData.field.name].name=="next_step"}}
				...
				{counter name="fieldsUsed"}
				{{if empty($colData.field.hideLabel)}}
				<div class="col-sm-2" scope="col" style="text-align:left" >
				{{if !empty($colData.field.name)}}
				{if !$fields.{{$colData.field.name}}.hidden}
				{{/if}}
				{{if isset($colData.field.customLabel)}}
...
{{if $module=="Prospects" || $module=="Leads" || $module=="Contacts" || $module=="Accounts" || $module=="Opportunities" || $module=="Cases"}}

<div class="col-sm-7 " style="border-right: 1px solid #d9dada;padding:0px">

{{else}}
<div class="col-sm-12 " style="border-right: 1px solid #d9dada;padding:0px">
{{/if}}
...
>
    ...
    {{/if}}
    <div {{if $useTabs}}class="yui-content"{{/if}} style="min-height:350px">
{{* Loop through all top level panels first *}}
{{counter name="panelCount" print=false start=0 assign="panelCount"}}
{{counter name="tabCount" start=-1 print=false assign="tabCount"}}
{{foreach name=section from=$sectionPanels key=label item=panel}}
...
{{else}}
    ...
    {{/if}}
	{{* Print out the table data *}}
  <table id='{{$label}}' class="panelContainer" cellspacing='{$gridline}' style="background-color:white;" >
	...
	<tr>
	    ...
	    {{/if}}
			{counter name="fieldsUsed"}
			{{if empty($colData.field.hideLabel)}}
			<td width='{{$def.templateMeta.widths[$smarty.foreach.colIteration.index].label+$def.templateMeta.widths[$smarty.foreach.colIteration.index].field}}%' scope="col" style="text-align:left" >
				{{if !empty($colData.field.name)}}
				    {if !$fields.{{$colData.field.name}}.hidden}
                {{/if}}
				{{if isset($colData.field.customLabel)}}
...
-->
			...
			<div id="subpanel_activities" style="display:block"></div>
			...
			<div id="subpanel_history" style="display:block"></div>
